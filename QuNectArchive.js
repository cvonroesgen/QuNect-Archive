...
{
    var configParameters = new Array("key", "filerid", "archivedbid", "filefid", "reffid");
    var that = this;
    var qdb = new QuickBaseClient();
    this.qdb = qdb;
    that.csvArchiveCache = {};
    this.NameValues = qdb.parseQueryString(queryString);
    qdb.SetAppToken(this.NameValues.apptoken);
    if(!checkInputParameters())
    {
        return;
    }
    //need to make a div
    this.jDiv = $("<div></div>").hide();
    $("body").append(this.jDiv);

    var yesButtonClick = function()
        {
        try
            {
                if(that.jSelect && that.jSelect.val() != "")
                {
                    var xml = that.qdb.DoQuery(that.NameValues.dbid, that.jSelect.val() + "AND" + makeQuery([that.NameValues.reffid, "XEX", ""]), makeClist([that.NameValues.keyfid, that.NameValues.reffid]));
                    var records = qdb.selectNodes(xml, "/*/table/records/record");
                    if(!confirm("Are you sure you want to restore the " + records.length + " records in the report " + that.jSelect.find(":selected").text()))
                        {
                        return;
                        }
                    for(var i = 0; i < records.length; i++)
                    {
                        restoreFromArchive(getTextByFID(records[i], that.NameValues.reffid), getTextByFID(records[i], that.NameValues.keyfid));
                    }
                    alert("Your records have been restored!");
                    window.location.reload(true);
                    return;
                }
                else if(that.NameValues.filerid == "")
                {
                    alert("Your record has already been restored!");
                    that.jDiv.dialog('close');
                    return;
                }
                else
                {
                    restoreFromArchive(that.NameValues.filerid, that.NameValues.key);
                    if(!that.qdb.displayErrorAlert("Could not restore from archive "))
                    {
            ...
            {
                alert("If you're using Internet Explorer 10, try again with ActiveX filtering turned off.\nClick on the little icon in your address bar between the padlock icon and the refresh icon.");
            }
            that.jDiv.dialog('close');
        };
    ...
    var noButtonClick = function(e)
        ...
        if(evt.shiftKey)
            {
            that.jSelect = $("<select><option value=''>Restore the record you clicked on.</option></select>");
            that.schema = that.qdb.GetSchema(that.NameValues.dbid);
            if(that.qdb.displayErrorAlert("Could not fetch schema "))
                {
                that.jDiv.dialog('close');
                }
            var qids = that.qdb.selectNodes(that.schema, "/*/table/queries/query[qycrit]");
            for(var i = 0; i < qids.length; i++)
                {
                var query = getNodeText(qids[i], "qycrit");
                var qname = getNodeText(qids[i], "qyname");
                var jOption = $("<option></option>");
                jOption.attr("value", query);
                jOption.text("Restore records in the report called: " + qname);
                that.jSelect.append(jOption);
                }
            that.jDiv.append(that.jSelect)
            }
        else
            {
            that.jDiv.dialog('close');
	...
	var title = 'Do you want to restore from the archive?';
	if(that.NameValues.filerid.match(/^[\{\(]/))
		{
		title = 'Do you want to restore all the records matching the query ' + that.NameValues.filerid + '?';
		}
    this.jDiv.dialog({
        title: title,
        autoOpen: true, 
        height: 200,
        width: 800, 
        buttons:[{text:"Yes", click:yesButtonClick}, {text:"No", click:noButtonClick}]
        });
    function restoreFromArchive(filerid, key)
    {
        if(filerid == "")
        {
            return;
        }
        if(that.csvArchiveCache[filerid] != undefined)
        {
            var strCSV = that.csvArchiveCache[filerid];
        }
        else
        {
            strCSV = that.qdb.GetFile(that.NameValues.archivedbid, filerid, that.NameValues.filefid);
            that.csvArchiveCache[filerid] = strCSV;
        }
        var csvArray = that.qdb.ParseDelimited(strCSV, ",");
        //now we need to go through and find the key value
        if(csvArray.length < 3)
        {
            alert("CSV archive " + filerid + " contains no rows.");
            return;
        }
        var numColumns = csvArray[0].length;
        var csvArrayRowNumber = 0;
        for(var i = 2; i < csvArray.length; i++)
        {
            if(csvArray[i][numColumns - 1] == key)
            {
                csvArrayRowNumber = i;
                break;
            }
        ...
        var rids = new Array();
        csvArray[0].push(that.NameValues.reffid);
        var numUpdated = that.qdb.ImportFromCSV(that.NameValues.dbid, makeCSVCells(csvArray[csvArrayRowNumber]), makeClist(csvArray[0]), rids, "0");
        if(that.qdb.errorcode == "31")
        {
            if(!that.schema)
                {
                //we need to do a GetSchema and then figure out what needs to be eliminated from the clist
                that.schema = that.qdb.GetSchema(that.NameValues.dbid);
                if(that.qdb.displayErrorAlert("Could not get schema. "))
                    {
                    return;
                    }
                }
            ...
            {
                if(!that.qdb.selectSingleNode(that.schema, "/*/table/fields/field[@id=" + csvArray[0][i] + "]"))
                {
                    csvArray[0][i] = "0";
                }
            }
            numUpdated = that.qdb.ImportFromCSV(that.NameValues.dbid, makeCSVCells(csvArray[csvArrayRowNumber]), makeClist(csvArray[0]), rids, "0");
        }
    }
    function checkInputParameters()
    {
        ...
        {
            if(!that.NameValues || !(configParameters[i] in that.NameValues) || that.NameValues[configParameters[i]] == "")
            {
                alert("Please make sure to specify a '" + configParameters[i] + "' in the query string.");
                return false;
            }
        }
        return true;
    ...
    function makeCSVCells(cArray)
    {
        ...
        for(var i in cArray)
        {
            ...
            if(cArray[i])
            {
                cell = cArray[i].toString().replace(/\"/g, "\"\"");
            }
            result += "\"" + cell + "\",";
        }
        return result;
 ...
 */
    ...
    function QuickBaseClient(qdbServer)
    {
        ...
        this.getCurrentUID = function()
        {
            var xmlQDBRequest = this.initXMLRequest();
            xmlQDBResponse = this.APIXMLPost("main", "API_GetOneTimeTicket", xmlQDBRequest);
            var uid = this.selectSingleNode(xmlQDBResponse, "/*/userid");
            if(uid)
            {
        ...
        this.GetFile = function(dbid, rid, fid)
        {
            var script = this.qdbServer + "/up/" + dbid + "/a/r" + rid + "/e" + fid + "/v0?apptoken=" + this.apptoken;
            xmlHTTPPost.open("GET", script, false);
            xmlHTTPPost.send(null);
            return xmlHTTPPost.responseText;
        }
    ...
    }//##################end of QuickBaseClient########################

};//#######################end of constructor for QuNectArchive
QuNectArchive.prototype.display = function(queryString)
{
    var queryNameValuePairs = queryString.split("&");
    for(var i = 0; i < queryNameValuePairs.length; i++)
    {
        var queryNameValuePair = queryNameValuePairs[i].split("=");
        this.NameValues[unescape(queryNameValuePair[0])] = unescape(queryNameValuePair[1]);
    }
    this.jDiv.dialog('open');
};
(function()
{
